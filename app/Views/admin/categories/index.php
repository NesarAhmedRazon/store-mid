...
<div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-5">
    ...
    <div class="flex items-center gap-2 w-full sm:w-auto">
        <input type="text" id="cat-search" placeholder="search categories..." class="flex-1 sm:w-52 text-[12px] font-mono px-3 py-1.5 border border-border rounded-md bg-transparent text-text placeholder:text-subtle focus:outline-none focus:border-border-md">
        <a href="/products/categories/import" class="shrink-0 text-[11px] font-mono text-subtle no-underline px-3 py-1.5 border border-border rounded-md hover:text-text hover:border-border-md transition-colors">
            ↑ import
        </a>
        <a href="/products/categories/create" class="shrink-0 text-[11px] font-mono text-text no-underline px-3 py-1.5 border border-border-md rounded-md hover:bg-bg transition-colors">
            + new
        </a>
    </div>
</div>
        ...
        <table class="w-full text-[12px]" id="cat-table">
            ...
            <tbody>
                ...
                <?php foreach ($categories as $cat): ?>
                    ...
                    ?>
                    <tr class="border-b border-border last:border-0 hover:bg-bg transition-colors cat-row" data-name="<?= esc(strtolower($cat->name)) ?>" data-slug="<?= esc(strtolower($cat->slug)) ?>">

                        <!-- Name with depth indent -->
                        <td class="px-4 py-2.5">
                            <div class="flex items-center gap-1.5" style="padding-left: <?= (int) $cat->depth * 18 ?>px">
                                <?php if ($cat->depth > 0): ?>
                                ...
                                <?php endif; ?>
                                <a href="/products/categories/<?= $cat->id ?>" class="text-text font-medium no-underline hover:underline">
                                    <?= esc($cat->name) ?>
                                </a> <?= $product_count > 0 ? '<sup>(' . $product_count . ')</sup>' : '' ?>
                            </div>
                        </td>
                        ...
                        <td class="px-4 py-2.5 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <a href="/products/categories/<?= $cat->id ?>" class="font-mono text-[11px] text-subtle no-underline hover:text-text transition-colors">view</a>
                                <a href="/products/categories/<?= $cat->id ?>/edit" class="font-mono text-[11px] text-subtle no-underline hover:text-text transition-colors">edit</a>
                                <button onclick="confirmDelete(<?= $cat->id ?>, '<?= esc($cat->name) ?>')" class="font-mono text-[11px] text-subtle hover:text-red-500 transition-colors bg-transparent border-none cursor-pointer p-0">
                                    delete
                                </button>
                            </div>
                        </td>
...
<div id="delete-modal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/30">
    <div class="bg-surface border border-border rounded-lg p-6 w-80 shadow-sm">
        ...
        <div class="flex gap-2 justify-end">
            <button onclick="closeModal()" class="text-[11px] font-mono px-3 py-1.5 border border-border rounded-md text-muted hover:text-text hover:border-border-md transition-colors bg-transparent cursor-pointer">
                cancel
            </button>
            <a id="delete-confirm-btn" href="#" class="text-[11px] font-mono px-3 py-1.5 border border-red-400 rounded-md text-red-500 no-underline hover:bg-red-50 transition-colors">
                delete
            </a>
        </div>
    </div>
